...
local _GravityController = nil

-- Patch GravityController constructor để lưu reference
local oldNew = GravityController.new
GravityController.new = function(...)
    local instance = oldNew(...)
    _GravityController = instance
    return instance
end

-- Thêm hàm Destroy vào GravityController
function GravityController:Destroy()
    if self._Destroyed then return end
    self._Destroyed = true
    
    -- Disconnect all connections
    if self.JumpCon then pcall(function() self.JumpCon:Disconnect() end) end
    if self.DeathCon then pcall(function() self.DeathCon:Disconnect() end) end
    if self.SeatCon then pcall(function() self.SeatCon:Disconnect() end) end
    if self.HeartCon then pcall(function() self.HeartCon:Disconnect() end) end
    
    -- Unbind render step
    pcall(function() RunService:UnbindFromRenderStep("GravityStep") end)
    
    -- Destroy objects
    if self.Collider then pcall(function() self.Collider:Destroy() end) end
    if self.VForce then pcall(function() self.VForce:Destroy() end) end
    if self.Gyro then pcall(function() self.Gyro:Destroy() end) end
    if self.Floor then pcall(function() self.Floor:Destroy() end) end
    ...
    self.GravityUp = Vector3.new(0, 1, 0)
    
    _GravityController = nil
end

function DisableWalkOnWall()
    if _GravityController then
        _GravityController:Destroy()
        return true
    end
    return false
end

function EnableWalkOnWall()
    if _GravityController then
        DisableWalkOnWall()
    end
    ...
    _GravityController = GravityController.new(player)
    return true
end

function ToggleWalkOnWall()
    if _GravityController then
        DisableWalkOnWall()
        return false
    else
...
function IsWalkOnWallEnabled()
    return _GravityController ~= nil
end

return {
    Enable = EnableWalkOnWall,
    Disable = DisableWalkOnWall,
    Toggle = ToggleWalkOnWall,
    IsEnabled = IsWalkOnWallEnabled,
    SetCameraSmoothness = function(value)
        if _GravityController then
            _GravityController.CameraSmoothness = value
            return true
        end
        return false
    end
